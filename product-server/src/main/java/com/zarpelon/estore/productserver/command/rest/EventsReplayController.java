...
public class EventsReplayController {
    ...
    public ResponseEntity<String> replayEvents(@PathVariable String processorName){
        var tracking = eventProcessingConfiguration.eventProcessor(processorName, TrackingEventProcessor.class);

        if(tracking.isPresent()) {
            var eventProcessor = tracking.get();
            ...
            return ResponseEntity.badRequest()
                    .body(String.format("The event processor with a name [%s] is not a tracking event processor. Only Tracking event processor is supported", processorName));
        }
    }
}
